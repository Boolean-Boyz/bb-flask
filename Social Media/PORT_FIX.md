...
**Your backend is running on:** `localhost:8321` ✅
...
**Change the port from `8585` to `8321`:**
...
// AFTER (Correct):
const javaURI = "http://localhost:8321";
```
...
export const javaURI = "http://localhost:8321";  // ← Make sure it's 8321!
...
Edit `assets/js/api/config.js` and change `8585` → `8321`
...
```bash
curl http://localhost:8321/api/post/all
# Should return: []
```
...
| Service | Port | Status |
|---------|------|--------|
| **Flask Backend** | **8321** | ✅ Correct - Use this! |
...
1. **Backend is on 8321** - This is confirmed and working ✅
2. **Don't change backend** - It's correct!
3. **Change frontend config** - Update to port 8321
...
Once you update the port to `8321`:
...
```javascript
// Backend API Configuration
export const javaURI = "http://localhost:8321";
...
# Find and replace 8585 with 8321
find assets -name "*.js" -type f -exec sed -i '' 's/8585/8321/g' {} +
...
| What | Where | Change |
|------|-------|--------|
| **Backend** | Flask (running) | Port **8321** ✅ |
| **Frontend Config** | `config.js` | Change to **8321** ❌→✅ |
...
Just update that one line in your frontend config from `8585` to `8321` and everything will work!
