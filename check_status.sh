...
if lsof -ti:8321 > /dev/null 2>&1; then
    ...
    echo "   Process IDs: $(lsof -ti:8321 | tr '\n' ' ')"
else
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8321/api/post/all)

if [ "$HTTP_CODE" == "200" ]; then
    echo "✅ API is responding (HTTP $HTTP_CODE)"
    echo ""
    echo "📊 Sample response:"
    curl -s http://localhost:8321/api/post/all | head -100
...
echo "🌐 Access Points:"
echo "========================================"
echo "   API: http://localhost:8321/api/post/all"
